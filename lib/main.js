...
import iconvTranslation from './iconv-lite-to-libiconv.json';
...
// resolves alias chains like these:
// { msansi: 'cp1252', cp1252: 'windows1252'} => { msansi: 'windows1252'},
const buildAliases = (encodings) => {
  const ret = {};
  Object.keys(encodings).forEach((enc) => {
    let i = enc;
    while (typeof encodings[i] === 'string') {
      i = encodings[i];
    }
    ret[enc] = i;
  });
...
const iconvLiteAliases = buildAliases(iconvEncodings);

const translateEncodingName = (name) => {
  const canonical = iconvLiteAliases[name] || name;
  if (iconvTranslation[canonical]) {
    return iconvTranslation[canonical];
  }
  // eslint-disable-next-line no-console
  console.warn(`linter-phpcs:: Potentially unsupported encoding '${canonical}', but trying anyway`);
  return canonical;
};
    ...
    return {
      ...
      lint: async (textEditor) => {
        ...
        if (version.major > 1
          ...
          const encoding = translateEncodingName(textEditor.getBuffer().getEncoding());
          parameters.push(`--encoding=${encoding}`);
        }
